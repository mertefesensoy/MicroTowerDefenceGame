...
public final class RelicSystem: @unchecked Sendable {
    ...
    // MARK: - GameState Compatibility / Adapter
    
    // 1. Convenience Init
    public convenience init(relicDefs: RelicDefinitions, rng: SeededRNG) {
        let db = RelicDatabase(definitions: relicDefs)
        self.init(db: db, rng: rng)
    }
    
    // 2. Generate Choices returning Objects
    public func generateChoices(count: Int) -> [RelicDef] {
        let ids = makeOfferIDs(count: count, excludeOwned: true)
        return ids.compactMap { db.relic(id: $0) }
    }
    ...
    public func combinedMultiplier(for type: RelicEffect.EffectType) -> Double {
        switch type {
        case .towerDamageMultiplier: return towerDamageMultiplier
        case .towerRangeMultiplier: return towerRangeMultiplier
        case .towerFireRateMultiplier: return towerFireRateMultiplier
        case .coinMultiplier: return coinMultiplier
        default: return 1.0
        }
    }
    
    // 5. Combined Additive
    public func combinedAdditiveValue(for type: RelicEffect.EffectType) -> Double {
        switch type {
        case .enemySlowOnHit: return enemySlowOnHit
        default: return 0.0
        }
    }
}
